...
from featureenginnering import QueProc,ProProc
...
class Recommandation:
    def __init__(self, model, que_data, stu_data, pro_data, que_to_stu, pos_pairs,
        ...
        self.que_dict = {row.values[0]: row.values[2:]
                         for i, row in que_data.iterrows()}
        self.stu_dict = {stu: group.values[-1, 2:]
                         for stu, group in stu_data.groupby('students_id')}
        self.pro_dict = {pro: group.values[-1, 2:]
                         for pro, group in pro_data.groupby('professionals_id')}
        ...
        for que in self.que_dict.keys():
            ...
            if cur_stu in self.stu_dict:
                ...
                que_feat.append(
                    np.hstack([self.stu_dict[cur_stu], self.que_dict[que]]))
                que_ids.append(que)
...
#         self.que_feat = self.que_feat_model.predict([que_feat])
    ...
    def __get_que_feat(self, que_df, que_tags):
        ...
        """
        que_df['questions_date_added'] = pd.to_datetime(
            que_df['questions_date_added'])

        # extract and preprocess question's features
        que_feat = self.que_proc.transform(que_df, que_tags).values[:, 2:]

        # actual question's features are both question and student's features
        stu_feat = np.vstack([self.stu_dict[stu]
                              for stu in que_df['questions_author_id']])
...
#         que_feat = self.que_feat_model.predict(que_feat)
    ...
    def __get_pro_feat(self, que_df, pro_df, ans_df, pro_tags):
        """
            this function takes a question from a particular professional and
            extracts it's features
        """

        pro_df['professionals_date_joined'] = pd.to_datetime(
            pro_df['professionals_date_joined'])
        que_df['questions_date_added'] = pd.to_datetime(
            que_df['questions_date_added'])
        ans_df['answers_date_added'] = pd.to_datetime(
            ans_df['answers_date_added'])
...
#         pro_feat = self.pro_feat_model.predict(pro_feat)
    ...
    def __dict_to_df(self, que_dict):
        ...
        que_df = df[['questions_id', 'questions_author_id',
                     'questions_date_added', 'questions_title', 'questions_body']]
        tags_df = df[['questions_id', 'questions_tags']].rename(
            columns={'questions_id': 'tag_questions_question_id',
                     'questions_tags': 'tags_tag_name'}
        )

        tags_df['tags_tag_name'] = tags_df['tags_tag_name'].apply(
            lambda x: tp.process(x, allow_stopwords=True))
        ...
        que_df['questions_whole'] = que_df['questions_title'] + \
            ' ' + que_df['questions_body']
    ...
    def __convert_pro_dict(self, pro_dict):
        ...
        pro_tags = pd.DataFrame(
            tuples, columns=['tag_users_user_id', 'tags_tag_name'])
        pro_df.drop(columns='professionals_subscribed_tags', inplace=True)

        pro_tags['tags_tag_name'] = pro_tags['tags_tag_name'].apply(
            lambda x: tp.process(x, allow_stopwords=True))
        pro_df['professionals_headline'] = pro_df['professionals_headline'].apply(
            tp.process)
        pro_df['professionals_industry'] = pro_df['professionals_industry'].apply(
            tp.process)
    ...
    def find_similar_questions(self, que_dict, top=10):
        """
            this function takes a question dict and find the top similar
            questions
        """
        ...
        similar_questions = pd.DataFrame(list(zip(ques_ids[0], dists[0])), columns=[
                                         'matched_id', 'distance'])
        return similar_questions

    def find_prof_by_question(self, que_dict, top=10):
        """
            this function takes a question dict and find the top professionals able 
...
#         top_pro = pd.DataFrame(list(zip(pro_ids[0], dists[0])), columns=['matched_pro_id','distance'])
        ...
        for ele in self.pro_feat:
            prob.append(self.model.predict(
                [que_feat.reshape(1, -1), ele.reshape(1, -1)])[0][0])
        prob = np.array(prob)
        indices = self.pro_ids[np.argsort(prob)[-top:]]
        top_pro = pd.DataFrame(list(zip(indices, prob[np.argsort(
            prob)[-top:]])), columns=['matched_pro_id', 'probability'])
        return top_pro
    ...
    def find_similar_professionals(self, prof_dict, que_df, ans_df, top=10):
        ...
        similar_pros = pd.DataFrame(list(zip(pro_ids[0][1:], dists[0][1:])), columns=[
                                    'matched_pro_id', 'distance'])
        return similar_pros
